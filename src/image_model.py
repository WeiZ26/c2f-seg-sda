...
class C2F_Seg(nn.Module):
    ...
    def batch_predict_maskgit(self, meta, iter, mode, T=3, start_iter=0):
        ...
        try:
            ...
            # 如果_未定义，从src_indices推断形状
            temp_quant_z, _, _ = self.g_model.encode(meta['vm_crop'].float())
            bhwc = (temp_quant_z.shape[0], temp_quant_z.shape[2], temp_quant_z.shape[3], temp_quant_z.shape[1])
        ...
        for t in range(start_iter, T):
            ...
            unknown_map = (cur_ids == self.mask_token_idx)
            ...
            selected_probs = torch.where(unknown_map, selected_probs, torch.Tensor([np.inf]).to(logits.device))
